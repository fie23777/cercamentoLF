			...
			for (int j = cont; j <= dois2; j++) {
				...
				for (int n = cont; n <= tres3; n++) {
					...
					for (int m = cont; m <= quatro4; m++) {
						...
						EsqDoisModel numerosCercados = new EsqDoisModel();
						numerosCercados.setNumEsqDois(numeros);
                        listEsqDois.add(numerosCercados);						
					}
				}
			}
		}
		numerosCercadoDao.gravarEsqDois(listEsqDois);
	}
